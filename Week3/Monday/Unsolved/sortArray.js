...
var b;          //This is a placeholder so we can swap the array values if needed
...
var bubbleSort = function(){
  ...
  while(swapped){
    ...
    for(var x = 0; x < numbers.length - 1; x++){
      ...
      var y = x + 1;  //y is the element in front of x

      if(numbers[x] > numbers [y]){
        b = numbers[x];                      //Put our current element into a placeholder for later
        numbers[x] = numbers[y];            //Set the current (Xth) element to our upcoming element (Yth)
        numbers[y] = b;                     //Set the upcoming element to our placeholder (the previous element)
        swapped = true;
      }
...
document.getElementById('button').onclick = function() {                  //When the button is clicked..
    document.getElementById("result").innerHTML = bubbleSort();    //Print bubblesort to the html
};


//How do we check if we need to do the bubbble sort?
//Do the bubble sort until
